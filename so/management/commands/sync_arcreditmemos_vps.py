"""
VPS management command: Sync AR credit memos from SAP API and save to local DB.
Runs on VPS when SSH tunnel exposes SAP API at localhost:8443.

Usage:
    python manage.py sync_arcreditmemos_vps
    python manage.py sync_arcreditmemos_vps --days-back 7
    python manage.py sync_arcreditmemos_vps --specific-date 2026-01-21
    python manage.py sync_arcreditmemos_vps --docnum 12345
"""
import sys
...
from so.sync_services import sync_arcreditmemos_core
...
logger = logging.getLogger('sync_arcreditmemos_vps')
...
class Command(BaseCommand):
    help = 'Sync AR credit memos from SAP API to local DB (runs on VPS via tunnel)'
    ...
    def handle(self, *args, **options):
        ...
        logger.info('=' * 70)
        logger.info('SAP AR Credit Memo Sync (VPS)')
        logger.info('=' * 70)
        logger.info(f'Started at: {sync_start.strftime("%Y-%m-%d %H:%M:%S")}')
        logger.info(f'API: {getattr(settings, "SAP_AR_CREDIT_MEMO_API_URL", "")}')
        ...
        try:
            stats = sync_arcreditmemos_core(days_back=days_back, specific_date=specific_date, docnum=docnum)
