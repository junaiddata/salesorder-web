...
# -*- coding: utf-8 -*-
"""
PC-based sync script to fetch AR Credit Memos from SAP API and push to VPS.
This script runs on your local PC and can access http://192.168.1.103/IntegrationApi/api/ARCreditMemo
...
from so.models import SAPARCreditMemo, SAPARCreditMemoItem
...
LOG_FILE = LOG_DIR / 'sync_arcreditmemos.log'
...
# Configure logging
logger = logging.getLogger('sync_arcreditmemos')
...
class Command(BaseCommand):
    help = 'Sync AR Credit Memos from SAP API to VPS via HTTP API (runs on PC, sends to VPS)'
    ...
    def handle(self, *args, **options):
        ...
        logger.info('=' * 70)
        logger.info('SAP AR Credit Memo Sync (PC -> VPS via HTTP)')
        logger.info('=' * 70)
        logger.info(f'Started at: {sync_start_time.strftime("%Y-%m-%d %H:%M:%S")}')
        logger.info(f'Local API: http://192.168.1.103/IntegrationApi/api/ARCreditMemo')
        ...
        self.stdout.write(self.style.SUCCESS('SAP AR Credit Memo Sync (PC -> VPS via HTTP)'))
        self.stdout.write(self.style.SUCCESS('=' * 70))
        self.stdout.write(f'Local API: http://192.168.1.103/IntegrationApi/api/ARCreditMemo')
        ...
        try:
            if docnum:
                ...
                logger.info(f'  Fetching AR credit memo by DocNum: {docnum}...')
                self.stdout.write(f'  Fetching AR credit memo by DocNum: {docnum}...')
                ...
                all_creditmemos = [cm for cm in creditmemos if str(cm.get('DocNum', '')) == str(docnum)]
                api_calls = 1
                logger.info(f'  Found {len(all_creditmemos)} credit memos for DocNum {docnum}')
            elif from_date and to_date:
                # Date range query
                logger.info(f'  Fetching AR credit memos for date range: {from_date} to {to_date}...')
                self.stdout.write(f'  Fetching AR credit memos for date range: {from_date} to {to_date}...')
                creditmemos = client.fetch_arcreditmemos_by_date_range(from_date, to_date)
                all_creditmemos.extend(creditmemos)
                api_calls = 1
                logger.info(f'  Found {len(creditmemos)} credit memos for date range {from_date} to {to_date}')
            elif specific_date:
                # Single date query
                logger.info(f'  Fetching AR credit memos for date: {specific_date}...')
                self.stdout.write(f'  Fetching AR credit memos for date: {specific_date}...')
                creditmemos = client.fetch_arcreditmemos_by_date_range(specific_date, specific_date)
                all_creditmemos.extend(creditmemos)
                api_calls = 1
                logger.info(f'  Found {len(creditmemos)} credit memos for date {specific_date}')
            else:
                # Default: fetch last N days
                logger.info(f'  Fetching AR credit memos for last {days_back} days...')
                self.stdout.write(f'  Fetching AR credit memos for last {days_back} days...')
                all_creditmemos = client.fetch_arcreditmemos_last_n_days(days=days_back)
                api_calls = 1
                logger.info(f'  Found {len(all_creditmemos)} credit memos')
            
            if not all_creditmemos:
                logger.warning('  No AR credit memos found.')
                self.stdout.write(self.style.WARNING('  No AR credit memos found.'))
                return
            ...
            for api_creditmemo in all_creditmemos:
                try:
                    ...
                    error_msg = f"Error mapping credit memo {api_creditmemo.get('DocNum')}: {e}"
                    logger.error(error_msg)
                    logger.exception(f"Error mapping credit memo {api_creditmemo.get('DocNum')}")
                    self.stdout.write(self.style.ERROR(f"  {error_msg}"))
                    mapping_errors.append(error_msg)
            
            if not mapped_creditmemos:
                logger.error('  No credit memos could be mapped successfully.')
                self.stdout.write(self.style.ERROR('  No credit memos could be mapped successfully.'))
                return
            ...
            def serialize_creditmemo(creditmemo):
                """Convert date objects to strings for JSON serialization"""
                serialized = creditmemo.copy()
            ...
            serialized_creditmemos = [serialize_creditmemo(cm) for cm in mapped_creditmemos]
            ...
            if local_only:
                ...
                # Batch size: send 200 credit memos per request to avoid exceeding DATA_UPLOAD_MAX_MEMORY_SIZE
                BATCH_SIZE = 200
                total_batches = (len(serialized_creditmemos) + BATCH_SIZE - 1) // BATCH_SIZE
                
                logger.info(f'  Sending {len(serialized_creditmemos)} credit memos in {total_batches} batches (batch size: {BATCH_SIZE})...')
                self.stdout.write(f'  Sending {len(serialized_creditmemos)} credit memos in {total_batches} batches (batch size: {BATCH_SIZE})...')
                
                vps_url = f"{VPS_BASE_URL}/saparcreditmemos/sync-api-receive/"
                total_created = 0
                total_updated = 0
                total_items = 0
                failed_batches = 0
                ...
                try:
                    for batch_idx in range(total_batches):
                        ...
                        end_idx = min(start_idx + BATCH_SIZE, len(serialized_creditmemos))
                        batch_creditmemos = serialized_creditmemos[start_idx:end_idx]
                        batch_creditmemo_numbers = [cm['credit_memo_number'] for cm in batch_creditmemos if cm.get('credit_memo_number')]
                        
                        logger.info(f'  Batch {batch_idx + 1}/{total_batches}: Sending {len(batch_creditmemos)} credit memos...')
                        self.stdout.write(f'  Batch {batch_idx + 1}/{total_batches}: Sending {len(batch_creditmemos)} credit memos...')
                        
                        payload = {
                            "creditmemos": batch_creditmemos,
                            "api_creditmemo_numbers": batch_creditmemo_numbers,
                            "api_key": VPS_API_KEY
                        }
            ...
            logger.info(f'Total Credit Memos Processed: {len(mapped_creditmemos)}')
            if local_only:
            ...
            self.stdout.write(f'Total Credit Memos Processed: {len(mapped_creditmemos)}')
            if local_only:
