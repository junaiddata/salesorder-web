...
def sync_quotations_from_api(request):
    ...
    if request.method == 'POST':
        try:
            ...
            # Fetch data from API (open quotations last 15 pages + last N days)
            logger.info(f"Starting quotation sync: days_back={days_back}")
            all_quotations = client.sync_all_quotations(days_back=days_back)
            sync_stats['api_calls'] = 1 + days_back  # 1 for open quotations + N for days
            ...
            if not mapped_quotations:
                messages.error(request, "No quotations could be mapped successfully.")
                return render(request, 'quotes/upload_quotations.html', {
                    'messages': messages_list,
                    'sync_stats': sync_stats
                })
            ...
            with transaction.atomic():
                ...
                for mapped in mapped_quotations:
                    ...
                    defaults = {
                        ...
                        "document_total": _dec2(mapped.get('document_total', 0)),  # Subtotal without VAT
                ...
                # Close missing quotations (quotations that were open but not in API response)
                # Find quotations that were previously open but are NOT in the API response
