...
Usage:
    python sync_arcreditmemos_pc.py           # Runs every 7 minutes (background service)
    python sync_arcreditmemos_pc.py --once    # Single run (for testing)
    python sync_arcreditmemos_pc.py --days-back 8  # Run once with custom days
"""
...
LOG_FILE = os.path.join(LOG_DIR, "sync_arcreditmemos.log")
...
def serialize_creditmemo(creditmemo: Dict) -> Dict:
...
def sync_arcreditmemos(days_back: int = DEFAULT_DAYS_BACK) -> Dict[str, Any]:
    """
    Fetch AR Credit Memos from SAP API and sync to VPS.
    ...
    log_message("=" * 70)
    log_message("SAP AR Credit Memo Sync (PC -> VPS via HTTP)")
    log_message("=" * 70)
    log_message(f"Started at: {sync_start_time.strftime('%Y-%m-%d %H:%M:%S')}")
    log_message(f"Local API: http://192.168.1.103/IntegrationApi/api/ARCreditMemo")
    ...
    try:
        ...
        # Fetch last N days
        log_message(f"  Fetching AR credit memos for last {days_back} days...")
        all_creditmemos = client.fetch_arcreditmemos_last_n_days(days=days_back)
        api_calls = 1
        log_message(f"  Found {len(all_creditmemos)} credit memos")
        
        if not all_creditmemos:
            log_message("  No AR credit memos found.", level="WARNING")
            return {"success": True, "stats": {"total": 0}, "error": None}
        
        log_message(f"  ✓ Fetched {len(all_creditmemos)} credit memos")
        ...
        if not mapped_creditmemos:
            log_message("  No credit memos could be mapped successfully.", level="ERROR")
            return {"success": False, "error": "No credit memos could be mapped"}
        
        if mapping_errors:
            log_message(f"  {len(mapping_errors)} credit memos failed to map", level="WARNING")
        
        log_message(f"  ✓ Mapped {len(mapped_creditmemos)} credit memos")
        
        # Get credit memo numbers
        ...
        log_message(f"  Sending {len(serialized_creditmemos)} credit memos to VPS...")
        ...
        if success:
            ...
            log_message(f"Total Credit Memos: {len(mapped_creditmemos)}")
            log_message("=" * 70)
            ...
            return {
                ...
                "stats": {
                    ...
                    "total_creditmemos": len(mapped_creditmemos),
                    "api_calls": api_calls,
...
def run_scheduled_sync():
    ...
    try:
        result = sync_arcreditmemos(days_back=SCHEDULED_DAYS_BACK)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Sync AR Credit Memos from SAP API to VPS",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python sync_arcreditmemos_pc.py               # Runs every 7 minutes (service mode)
  python sync_arcreditmemos_pc.py --once        # Single run
  python sync_arcreditmemos_pc.py --days-back 8 # Run once with 8 days
  python sync_arcreditmemos_pc.py --days-back 8 --service  # Run every 7 min with 8 days
        """
    )
    parser.add_argument(
        '--once',
    ...
    if run_once:
        ...
        result = sync_arcreditmemos(days_back=args.days_back)
        ...
        log_message("=" * 70)
        log_message("PC AR Credit Memo Sync Service Started")
        log_message("=" * 70)
