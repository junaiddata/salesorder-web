"""
Periodic management command: poll the Outlook/IMAP inbox (sales@junaid.ae),
classify new emails as RFQ / not relevant / needs manual review, and store
them. Mirrors poll_gmail.py -- see emailagent/outlook_client.py and
emailagent/services.py:poll_outlook for the mechanics.

Usage:
    python manage.py poll_outlook
    python manage.py poll_outlook --dry-run
    python manage.py poll_outlook --max-results 20
"""
import sys
...
LOG_FILE = LOG_DIR / 'poll_outlook.log'
LOG_MAX_BYTES = 10 * 1024 * 1024  # 10 MB
LOG_BACKUP_COUNT = 5

LOG_DIR.mkdir(exist_ok=True)

logger = logging.getLogger('poll_outlook')
...
class Command(BaseCommand):
    help = 'Poll the Outlook/IMAP mailbox for new emails and classify them as RFQ / not relevant / needs review'
    ...
    def handle(self, *args, **options):
        ...
        logger.info('=' * 70)
        logger.info('Outlook Poll (Email Tracking Agent)')
        logger.info('=' * 70)
        ...
        try:
            stats = poll_outlook(dry_run=dry_run, max_results=max_results)
        except Exception:
            logger.exception('Error during Outlook poll')
            raise SystemExit(1)
