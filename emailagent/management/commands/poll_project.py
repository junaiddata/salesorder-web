...
Usage:
    ...
    python manage.py poll_project --dry-run
    python manage.py poll_project --max-results 20
"""
import sys
...
LOG_FILE = LOG_DIR / 'poll_project.log'
LOG_MAX_BYTES = 10 * 1024 * 1024  # 10 MB
LOG_BACKUP_COUNT = 5

LOG_DIR.mkdir(exist_ok=True)

logger = logging.getLogger('poll_project')
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            stats = poll_project_mailbox(dry_run=dry_run, max_results=max_results)
        except Exception:
            logger.exception('Error during project mailbox poll')
            raise SystemExit(1)
