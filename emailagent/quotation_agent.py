...
_ITEM_MATCH_RETRY_RULES = (
    ...
    "matches as a whole.\n\n"
    "JOINT TYPE -- push-fit vs solvent. UPVC drainage/soil pipes and fittings "
    "are sold with one of two joint types, and the customer's wording is not "
    "the catalog's:\n"
    "  push fit / push-fit / rubber ring / RR / PF  ->  catalog writes RR, PF, "
    "RUBBER RING or PUSH FIT\n"
    "  solvent / solvent weld / glue type / SS / PE / plain end / plane end  "
    "->  catalog writes SS, SOLVENT, P/E or PLAIN END (\"plane end\" is a "
    "misspelling of plain end and is not in the catalog)\n"
    "When a requirement item states a joint type, put the customer's "
    "joint-type word in the lookup_item_master search (\"cosmo elbow 4 push "
    "fit\", \"hepworth pipe 4 solvent\") -- the tool matches every catalog "
    "spelling and ranks that joint type first -- and quote ONLY an item of "
    "that joint type. If the item exists only in the OTHER joint type (or the "
    "right joint type has no such size), do not quote it: leave item_code "
    "empty and offer it in suggested_substitutes, naming the difference "
    "(\"SS solvent-weld instead of the push-fit requested\"). These codes "
    "mean joint types ONLY on UPVC pipes and fittings -- RR on electrical "
    "cable, SS meaning stainless steel and PE meaning polyethylene in PPR/HDPE "
    "items are not joint types, so ignore them. When the requirement names no "
    "joint type this rule does not apply (the Cosmo end cap / access plug SS "
    "default above still stands). Do not add glue or solvent cement lines "
    "unless the customer asked for them.\n\n"
